...
- [Messages API](/foundations/messages-api) — the underlying API that tool use builds on
- [Streaming](/foundations/streaming) — streaming responses including tool use events
- [Structured Output](/foundations/structured-output) — using tools to force JSON output
- [Agent Architecture](/agents/agent-architecture) — building agents from agentic tool loops
- [MCP](/agents/mcp) — standardized tool protocol for interoperable tools
