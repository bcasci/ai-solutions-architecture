...
Prompt engineering without evaluation is guesswork. An eval pipeline gives you objective scores so you can compare prompt versions, catch regressions, and ship with confidence. This page covers the full workflow: building test datasets, running evals, and grading with both code and model-based approaches.
...
Always choose option 3 for any prompt going to production. The upfront investment pays for itself the first time you catch a regression.
...
# Generate and save
dataset = generate_dataset("Python utility functions for string processing")
...
A model grader uses Claude as a judge to evaluate response quality. The key insight: ask for strengths, weaknesses, and reasoning *before* the score. Without this, model graders default to middling scores around 6.
...
:::warning
Model graders are variable between runs. Individual scores may differ, but average trends across a full dataset are stable enough for prompt comparison. Never draw conclusions from a single test case score.
:::

## Code-Based Grading

Code graders use programmatic checks for objective criteria: valid syntax, correct format, presence of required elements. They are deterministic, fast, and free.
...
A prompt that scores higher on average across your dataset is objectively better. This is the basis for confident, data-driven prompt development.
...
- [Structured Output](/foundations/structured-output) --- enforce output format for cleaner grading
- [System Prompts](/foundations/system-prompts) --- use system prompts to set evaluation criteria
- [Models & Parameters](/foundations/models-and-parameters) --- choose models for generation vs. grading
