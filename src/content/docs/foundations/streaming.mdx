...
:::tip
For [multi-turn conversations](/foundations/messages-api#multi-turn-conversations), call `stream.get_final_message()` (Python) or `stream.finalMessage()` (TypeScript) after streaming completes and append the result to your messages list. The assembled text is identical to what a non-streaming call returns.
:::
...
- [Messages API](/foundations/messages-api) — request structure and non-streaming responses
- [Structured Output](/foundations/structured-output) — parsing structured data from responses
