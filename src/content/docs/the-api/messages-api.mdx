...
message = client.messages.create(
    ...
    max_tokens=1024,
    messages=[
        {"role": "user", "content": "Extract the company name and total amount from this invoice:\n\nINVOICE #1042\nFrom: Acme Cloud Services LLC\nDate: 2025-03-15\nItems:\n  - API usage (March): $1,200.00\n  - Support tier (Pro): $300.00\nTotal: $1,500.00"}
    ]
)
...
| `system` | string or array | No | [System prompt](/prompting/system-prompts) — instructions separate from the conversation |
| `stop_sequences` | array of strings | No | Custom strings that halt generation. See [structured output](/prompting/structured-output). |
| `stream` | boolean | No | Enable [streaming](/the-api/streaming). Default `false`. |

Each message in the `messages` array has two fields: `role` (either `"user"` or `"assistant"`) and `content` (a string or array of content blocks for [multimodal input](/the-api/multimodal-input)).

:::tip
`max_tokens` is a safety ceiling, not a target. Claude stops when it finishes — it does not try to fill the limit. Set it high enough that responses won't be cut off, but don't worry about it inflating output.
:::
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "Company: Acme Cloud Services LLC, Total: $1,500.00"
    }
  ],
...
- **`stop_reason`** — Why generation stopped: `end_turn` (natural finish), `max_tokens` (hit the ceiling), `stop_sequence` (matched a custom stop string), or `tool_use` (invoked a [tool](/the-api/tool-use)).
...
# Turn 1
print(chat("Extract the company name and total from this invoice: INVOICE #1042, From: Acme Cloud Services LLC, Total: $1,500.00"))

# Turn 2 — Claude has context from turn 1
print(chat("Now also extract the payment terms and calculate the due date assuming net-30."))
```
...
For details on model context sizes, see [Models & Parameters](/the-api/models-and-parameters).
...
- [Models & Parameters](/the-api/models-and-parameters) — choosing the right model and tuning output
- [System Prompts & Prompt Design](/prompting/system-prompts) — shaping Claude's behavior
- [Streaming](/the-api/streaming) — real-time progressive responses
- [Structured Output](/prompting/structured-output) — getting JSON from Claude
