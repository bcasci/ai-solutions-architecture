...
with client.messages.stream(
    ...
    max_tokens=1024,
    messages=[{"role": "user", "content": "Write a migration guide for users upgrading from v2 to v3 of our REST API. Cover authentication changes, deprecated endpoints, and new rate limits."}]
) as stream:
...
stream = client.messages.create(
    model="claude-sonnet-4-20250514",
    max_tokens=1024,
    messages=[{"role": "user", "content": "Summarize the key risks and action items from this quarterly compliance report."}],
    stream=True,
)
