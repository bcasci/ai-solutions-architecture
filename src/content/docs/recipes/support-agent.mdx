...
description: "A complete support agent combining routing, RAG, tool use, multi-turn conversation, and streaming — every major pattern in one runnable script."
section: "recipes"
order: 5
...
def search_knowledge_base(query: str, top_k: int = 2) -> list[dict]:
    """Simple keyword search over help articles. Replace with vector search in production."""
...
<Callout type="info">
The router uses Haiku for speed and cost — classification doesn't need a large model. The specialized handlers use Sonnet for quality. This is a common pattern: [cheap model for routing, capable model for generation](/the-api/models-and-parameters).
</Callout>
...
def run_agent(user_message: str, conversation: list[dict] | None = None) -> str:
    ...
    return "I need to hand this off to a human agent — the issue is more complex than I can resolve."
```

## Full Runnable Script

Copy and paste the entire script. Set `ANTHROPIC_API_KEY` in your environment, then run it.
