...
- Response quality improves with specialized context
...
The two-step solution: generate first (accepting some violations), then pass the output to a dedicated revision prompt that focuses solely on checking and fixing those violations. This outperforms trying to get everything right in a single call.
