...
Start by asking three questions about your task:

1. **Does the input type determine what happens next?** Different input categories need different prompts, tools, or pipelines → **Route** first, then handle each category with the appropriate pattern.

2. **Do the steps depend on each other?** Step 2 needs the output of step 1 → **Chain** them sequentially. Each step gets a focused prompt that builds on the previous output.

3. **Are the subtasks independent?** Multiple evaluations, checks, or analyses that don't need each other's output → **Parallelize** and aggregate the results.

| Pattern | Choose when | Example | Trade-off |
| --- | --- | --- | --- |
| **Routing** | Input varies by type and each type needs different handling | Support tickets: billing → billing pipeline, technical → technical pipeline | Extra classification call, but each handler is specialized |
...
See [Choosing Your Strategy](/start-here/choosing-your-strategy) for the broader decision of when orchestration is the right approach vs. a single prompt or an agent loop.
