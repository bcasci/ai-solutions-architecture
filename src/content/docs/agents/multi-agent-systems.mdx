...
A single agent with the right tools handles most tasks. But some problems — code review pipelines, multi-domain analysis, parallel research — benefit from splitting work across multiple specialized agents. This page covers multi-agent architectures, when they're worth the complexity, and how to build them with both the Messages API and the managed agents API.
...
This is powerful for teams building agent systems: one MCP server wraps your internal APIs, and every agent — regardless of who built it — gets the same tool interface. See [MCP](/agents/mcp) for building servers and clients.
...
The AI-reviews-AI pattern — where a hook launches a separate Claude instance to review the first Claude's work — is a practical multi-agent pattern that runs inside your development workflow.
...
- [MCP (Model Context Protocol)](/agents/mcp) — building MCP servers for shared tool access
- [Claude Code Skills](/agents/claude-code-skills) — skills and subagent configuration
- [Claude Code Hooks](/agents/claude-code-hooks) — event-driven automation including AI-reviews-AI
