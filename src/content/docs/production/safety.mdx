...
Even with good guardrails, validate what comes back. Claude occasionally drifts outside intended boundaries.
...
This adds latency (two API calls) but provides structured moderation decisions. Use Haiku for the moderation call to minimize cost and latency.

## Structured Output for Safety

When Claude's output feeds into downstream systems, use [structured output](/prompting/structured-output) to constrain the response format. This prevents unexpected content from reaching systems that parse Claude's output programmatically.
...
- [System Prompts](/prompting/system-prompts) — system prompt design fundamentals
- [Structured Output](/prompting/structured-output) — constraining response format
- [Orchestration Patterns](/agents/orchestration) — using Claude for content classification
