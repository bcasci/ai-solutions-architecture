...
Claude charges per token — input and output. On high-volume applications, costs add up fast. Prompt caching, batch processing, and smart request design can cut your bill by 50-90% depending on your usage patterns. This page covers the mechanics and when each strategy pays off.

## How Prompt Caching Works

Every API request triggers preprocessing: tokenization, embedding, context analysis. Without caching, Claude discards all that work after responding. On the next request with the same content, it repeats everything from scratch.
...
Tool schemas are prime caching candidates — they rarely change between requests and can be large.
...
Batch processing returns results within 24 hours (typically much faster). Use it for evaluations, bulk classification, data extraction, and any non-interactive workload.
...
- [Messages API](/foundations/messages-api) — request structure and the usage object
- [Models & Parameters](/foundations/models-and-parameters) — model pricing tiers and capabilities
- [Tool Use](/patterns/tool-use) — building tool schemas that work with caching
