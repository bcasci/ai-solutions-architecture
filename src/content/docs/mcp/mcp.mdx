...
section: "mcp"
order: 1
tags: ["mcp", "tool-use", "tool-design", "protocols"]
lastVerified: "2026-06-01"
---
MCP (Model Context Protocol) is a standard communication layer that lets you give Claude access to external tools and data without custom integration code. Instead of defining tool schemas and execution functions yourself, you connect to MCP servers that provide them pre-built.

## When to Use MCP

If you've already seen [Tool Use](/the-api/tool-use), you know you can define tools directly in your API calls. MCP is an alternative path — here's when each makes sense:

| | Direct tool definitions | MCP |
| --- | --- | --- |
| **Setup** | Define JSON schemas inline in your API call | Run an MCP server, connect your client |
| **Best for** | App-specific tools, few in number, tightly coupled to your code | Reusable tools, shared across projects or teams |
| **Ecosystem** | You build everything | Pre-built servers exist for GitHub, databases, Slack, file systems, and more |
| **Overhead** | None — it's just API parameters | Server process + protocol layer |

**Start with direct tool definitions** if you're building one app with a handful of custom tools. **Move to MCP** when you find yourself duplicating tool definitions across projects, want to share tools across a team, or want to use pre-built MCP servers.

Many production systems use both: direct tools for app-specific logic, MCP for standard integrations.
...
The Python SDK (`mcp`) provides `FastMCP`, a class that decorates Python functions as MCP tools.
...
mcp = FastMCP("TicketMCP", log_level="ERROR")

# In-memory ticket store for demo purposes
tickets = {
    "TKT-1042": {"title": "Billing error on March invoice", "status": "open", "priority": "high", "customer": "alice@acme.com"},
    "TKT-1043": {"title": "API returns 500 on /v2/orders", "status": "open", "priority": "critical", "customer": "bob@startup.io"},
    "TKT-1044": {"title": "Request to upgrade plan", "status": "pending", "priority": "low", "customer": "carol@bigcorp.com"},
}

@mcp.tool(
    name="get_ticket",
    description="Retrieve a support ticket by ID. Returns the ticket's title, status, priority, and customer email."
)
def get_ticket(
...
@mcp.tool(
    ...
    description="Update the status of a support ticket. Valid statuses: open, pending, in_progress, resolved, closed."
)
def update_ticket_status(
    ticket_id: str = Field(description="The ticket ID to update"),
    new_status: str = Field(description="New status: open, pending, in_progress, resolved, or closed"),
) -> str:
    if ticket_id not in tickets:
        raise ValueError(f"Ticket {ticket_id} not found")
    valid = {"open", "pending", "in_progress", "resolved", "closed"}
    if new_status not in valid:
        raise ValueError(f"Invalid status '{new_status}'. Must be one of: {valid}")
    tickets[ticket_id]["status"] = new_status
    return f"Updated {ticket_id} status to {new_status}"
```
...
def list_tickets() -> list[dict]:
    return [{"ticket_id": tid, **t} for tid, t in tickets.items()]
...
Two resource types:
- **Direct** — static URI (`tickets://list`), no parameters. Use for enumerations and indexes.
- **Templated** — parameterized URI (`tickets://ticket/{ticket_id}`). The SDK extracts `{ticket_id}` from the URI and passes it as a keyword argument automatically.
...
@mcp.prompt(
    name="triage",
    description="Classify a support ticket's priority and suggest an owner based on the ticket content."
)
def triage_ticket(
    ticket_id: str = Field(description="The ticket ID to triage")
) -> list[base.Message]:
    return [
        base.UserMessage(
            f"Triage support ticket '{ticket_id}'. Use the 'get_ticket' tool to read its details, "
            f"then classify the priority as critical, high, medium, or low. Suggest which team "
            f"(billing, engineering, account-management) should own it. Use 'update_ticket_status' "
            f"to set the status to 'in_progress' once triaged."
        )
    ]
```
...
The MCP SDK includes a browser-based inspector for testing servers locally.
...
async def run():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            messages = [{"role": "user", "content": "What's the status of ticket TKT-1042?"}]
...
This page covers the foundation — if you're building servers, see [Building MCP Servers](/mcp/building-servers) for TypeScript examples, tool design patterns, and testing strategies. For client-side integration, multi-server coordination, and Claude Code setup, see [MCP Client Integration](/mcp/client-integration).

## Related

- [Building MCP Servers](/mcp/building-servers) — Python and TypeScript servers, tool design, testing
- [MCP Client Integration](/mcp/client-integration) — multi-server, Claude Code config, resource injection
